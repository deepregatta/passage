...
export function deriveChangeStory(changes, briefing) {
  ...
  return {
    headline_plain: changes?.verdict_transition?.from !== changes?.verdict_transition?.to ? `The assessment changed from ${label(changes.verdict_transition.from)} to ${label(changes.verdict_transition.to)}.` : `The assessment remains ${label(changes?.verdict_transition?.to ?? 'unchanged')}, with updated detail.`,
...
const label = (state) => ({ within: 'within your limits', approaching: 'close to your limits', exceeds: 'beyond your limits', insufficient: 'too uncertain to assess', warning_active: 'warning active' })[state] ?? String(state).replaceAll('_', ' ');
