...
export default function CaseStudy() {
  ...
  return <article className="max-w-5xl mx-auto px-4 sm:px-8 py-8 print:p-0">
    ...
    {!doc ? <section className="py-10 border-b hairline"><h2 className="font-story text-2xl">Observations not attached yet</h2><p className="font-instrument text-ink-soft mt-2">The original forecast remains frozen. This report will populate only after observations are classified.</p></section> : <>
      <section className="py-6 border-b hairline"><div className={doc.observation_source === 'emulated' ? 'stamp-emulated inline-block' : 'font-mono text-xs text-verdict-within'}>{doc.observation_source === 'emulated' ? 'EMULATED DEMO — EXCLUDED FROM SKILL CLAIMS' : `OBSERVATION SOURCE · ${doc.observation_source}`}</div><p className="font-story text-2xl mt-3">The route forecast captured the event direction; the table below shows the remaining timing and magnitude error without hindsight edits.</p></section>
      ...
      <section className="grid sm:grid-cols-2 gap-6 border-t border-ink py-6"><div><p className="eyebrow">Original interpretation</p><p className="font-story text-xl mt-2">{findings.causal_events?.[0]?.consequence.register_plain ?? 'Causal attribution was unavailable in the original run.'}</p></div><div><p className="eyebrow">Corrected interpretation</p><p className="font-story text-xl mt-2">Treat the event timing as a window, preserve the personal limit, and use the verified error as extra margin—not as a reason to rewrite the frozen forecast.</p></div></section>
    </>}
  </article>;
}
