...
describe('limit fragment boundaries', () => {
  it.each([
    ['limit', 'limite'],
    ['Limit: 18 kt', 'limite: 18 nd'],
    ['(LIMIT), limit.', '(limite), limite.'],
    ['limit / limits', 'limite / limites'],
    ['  limit\n', '  limite\n'],
    ['Couverture limitée ; limit: 18 kt', 'Couverture limitée ; limite: 18 nd'],
  ])('translates standalone English fragments: %s', (english, french) => {
    expect(translateText(english, 'fr')).toBe(french);
    expect(translateText(french, 'fr')).toBe(french);
    expect(translateText(english, 'en')).toBe(english);
  });

  it.each([
    'limitée', 'limité', 'limitées', 'limités', 'LIMITÉE',
    'Couverture limitée à la Manche.',
    'Couverture limitée à la Manche.'.normalize('NFD'),
    'illimité ; délimitée ; limite ; limites',
    'prélimit ; limit_value ; limit2 ; unlimited',
  ])('preserves French words and embedded tokens: %s', (text) => {
    expect(translateText(text, 'fr')).toBe(text);
  });

  it('preserves French generated by a sentence pattern before fragment replacement', () => {
    expect(translateText('Partial capability coverage: limited inputs.', 'fr'))
      .toBe('Couverture partielle des capacités : données d’entrée limitées.');
  });
});
...
describe('document localiser scope', () => {
  ...
  it('preserves accented French text and attributes through language switches', async () => {
    mount('en');
    const text = 'Couverture limitée ; limit: 18 kt';
    const french = 'Couverture limitée ; limite: 18 nd';
    const span = host.querySelector('span');
    span.textContent = text;
    span.title = text;
    for (const language of ['fr', 'en', 'fr']) {
      view.rerender(createElement(LocalizedDocument, { language }));
      await settle();
      expect(span.textContent).toBe(language === 'fr' ? french : text);
      expect(span.title).toBe(language === 'fr' ? french : text);
    }
  });
