...
def _times(departure: datetime) -> list[str]:
    ...
    return [
        (start + timedelta(hours=h)).strftime("%Y-%m-%dT%H:%M") for h in range(WINDOW_H)
    ]
            ...
            "bulletins": [
                {
                    ...
                    "kind": "BMS-large",
                    "severity": "gale",
                    "valid_from": (departure + timedelta(hours=6)).strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    ),
                    "valid_to": (departure + timedelta(hours=30)).strftime(
                        "%Y-%m-%dT%H:%M:%SZ"
                    ),
                    "raw_text": "SYNTHETIC BULLETIN — Gale warning: W or SW gale force 8 "
                    "expected later. Generated by the deepweather scenario harness; "
